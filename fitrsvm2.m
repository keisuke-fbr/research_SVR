function obj = fitrsvm2(X,Y,varargin)
...
%                            entry means that the corresponding predictor
...
%                        categorical.
%                          NOTE: FITRSVM creates dummy variables for
%                                categorical predictors by expanding each
%                                categorical predictor into a set of binary
%                                predictors. The process is different for
%                                unordered and ordered categorical
%                                predictors in the table TBL. For the
%                                matrix X, FITRSVM treats categorical
%                                predictors as unordered. For details, see
%                                the reference page of FITRSVM.
...
[IsOptimizing, RemainingArgs] = parseOptimizationArgs2(varargin);
if IsOptimizing
    obj = fitoptimizing2('fitrsvm2',X,Y,varargin{:});
else
    obj = RegressionSVM2.fit(X,Y,RemainingArgs{:});
end
end
