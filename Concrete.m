% 実データでの事例3 (実データ(コンクリートの強度))

data = readmatrix('Concrete.csv'); % コンクリートに関するデータ(1030個)
X = [data(:, 1) data(:, 4), data(:, 6), data(:, 7)]; % セメントの量, 水の量, 粗骨材の量, 細骨材の量
Y = data(:, 9); % コンクリートの強度
...
% title("L1損失SVR");
% saveas(gcf, 'L1損失SVRによるコンクリート強度の予測精度.jpg') % このコードでグラフを画像ファイルとして保存
% %}
...
xlabel('true', 'FontSize',18); % x軸ラベルの大きさ
ylabel('pred', 'FontSize',18); % y軸ラベルの大きさ
% title("線形εインセンシティブ損失関数を用いたSVRによるコンクリート強度の予測精度2");
% saveas(gcf, 'L1損失SVRによるコンクリート強度の予測精度2.jpg') % このコードでグラフを画像ファイルとして保存
%}
% %}
...
ylabel('pred', 'FontSize',18); % y軸ラベルの大きさ
% title("L2損失SVR");
% saveas(gcf, 'L2損失SVRによるコンクリート強度の予測精度.jpg') % このコードでグラフを画像ファイルとして保存
% %}
...
xlabel('true', 'FontSize',18); % x軸ラベルの大きさ
ylabel('pred', 'FontSize',18); % y軸ラベルの大きさ
% title("2次εインセンシティブ損失関数を用いたSVRによるコンクリート強度の予測精度2");
% saveas(gcf, 'L2損失SVRによるコンクリート強度の予測精度2.jpg') % このコードでグラフを画像ファイルとして保存
%}
% %}