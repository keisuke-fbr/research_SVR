...
% saveas(gcf, '例に用いた人工データ(非線形).eps', 'epsc') % このコードでグラフを画像ファイルとして保存
% saveas(gcf, '例に用いた人工データ(非線形).jpg') % このコードでグラフを画像ファイルとして保存
% %}
...
epsilon = 1;
C = 10;
KernelScale = 1/sqrt(10); % Pythonにおけるgamma (gamma = 1/KernelScale^2). gamma =[0.1, 1, 10, 100]と等価
% %}

mdl_L1SVR = fitrsvm(X, Y_noize, 'BoxConstraint', C, 'KernelFunction', 'gaussian', 'KernelScale', KernelScale, 'Epsilon', epsilon);
...
xlabel('$x$', 'FontSize',18, 'Interpreter', 'latex'); % x軸ラベルの大きさ
ylabel('$y$', 'FontSize',18, 'Interpreter', 'latex'); % y軸ラベルの大きさ
% saveas(gcf, 'サポートベクターが多いとき(非線形).eps', 'epsc') % このコードでグラフを画像ファイルとして保存
% saveas(gcf, 'サポートベクターが多いとき(非線形).jpg') % このコードでグラフを画像ファイルとして保存

% サポートベクターの数を表示
disp("L1損失非線形SVRのサポートベクターの数(epsilon=1, サポートベクターが多いとき)");
...
xlabel('$x$', 'FontSize',18, 'Interpreter', 'latex'); % x軸ラベルの大きさ
ylabel('$y$', 'FontSize',18, 'Interpreter', 'latex'); % y軸ラベルの大きさ
% saveas(gcf, 'サポートベクターが少ないとき(非線形).eps', 'epsc') % このコードでグラフを画像ファイルとして保存
% saveas(gcf, 'サポートベクターが少ないとき(非線形).jpg') % このコードでグラフを画像ファイルとして保存

% サポートベクターの数を表示
disp("L1損失非線形SVRのサポートベクターの数(epsilon=4, サポートベクターを少ないとき)");
length(mdl_L1SVR.SupportVectors)