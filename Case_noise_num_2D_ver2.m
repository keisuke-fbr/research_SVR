% 人工データ(2次元) 大きな変動がホワイトノイズに依存する場合
% 曲線上にデータを生成し, 以下の二つの条件を併せて考えて, 予測精度を比較
% (1) ノイズを変化(ホワイトノイズの標準偏差を小さくしたい大きくしたり ⇒ 最終的には5に固定した)
% (2) データ数を変化(データ数が少ない場合はノイズの影響は大きく, 多い場合は小さくなる)

num = [100, 500, 1000, 2000, 3000, 4000];
std = [5]; % ホワイトノイズの標準偏差 ⇒ 最終的には5にした
X_range = 5; % Xの生成範囲の上限
...
for num_i = num
    ...
    for std_i = std
        ...
        % disp(append('(データ数=', string(num_i), ', ホワイトノイズの標準偏差=', string(std_i), 'のとき)'));
        ...
        xlabel('$x$', 'FontSize',18,'Interpreter','latex'); % x軸ラベルの大きさ
        ylabel('$y$', 'FontSize',18,'Interpreter','latex'); % y軸ラベルの大きさ
        % saveas(gcf, append('人工データ(データ数=', string(num_i), ' ホワイトノイズの標準偏差=', string(std_i), 'のとき).jpg')) % このコードでグラフを画像ファイルとして保存
        % %}
        ...
        % saveas(gcf, append('L1損失SVRの予測精度(データ数=', string(num_i), ', ホワイトノイズの標準偏差=', string(std_i), 'のとき).jpg')) % このコードでグラフを画像ファイルとして保存
        % %}

        disp(append('L1損失SVRの予測精度(データ数=', string(num_i), ', ホワイトノイズの標準偏差=', string(std_i), 'のとき)'));
        disp("真値との平均絶対値誤差");
        ...
        ylabel('pred', 'FontSize',18); % y軸ラベルの大きさ
        % title("L2損失SVR");
        % saveas(gcf, append('L2損失SVRの予測精度(データ数=', string(num_i), ' ホワイトノイズの標準偏差=', string(std_i), 'のとき).jpg')) % このコードでグラフを画像ファイルとして保存
        % %}

        disp(append('L2損失SVRの予測精度(データ数=', string(num_i), ' ホワイトノイズの標準偏差=', string(std_i), 'のとき)'));
        disp("真値との平均絶対値誤差");
