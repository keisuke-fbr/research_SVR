...
%   Copyright 2010-2018 The MathWorks, Inc.
    ...
    methods(Access=protected)
        function this = CompactEnsemble2(usepredforlearner)
        ...
        function s = propsForDisp(this,s)
            if nargin<2 || isempty(s)
                ...
                if ~isstruct(s)
                    error(message('CompactEnsemble2:propsForDisp:BadS'));
                end
            end
    ...
    methods
        ...
        function this = set.CombineWeights(this,cw)
            ...
            elseif strcmpi(cw,'weightedaverage')
                ...
                error(message('CompactEnsemble2:setCombineWeights:BadArgs'));
            end
        end
    ...
    methods(Static,Hidden)
        function score = aggregatePredict(X,combiner,trained,...
            ...
            [useNfort,useDfort,learnerIdx,mode] = ...
                CompactEnsemble2.checkAggregateArgs(T,N,D,varargin{:});
            
            % Only the default 'ensemble' mode is allowed
            if strncmpi(mode,'ensemble',length(mode))==0
                error(message('CompactEnsemble2:aggregatePredict:BadMode'));
            end

            % Check useNfort
            if ~islogical(useNfort) || ~all([N T]==size(useNfort))
                error(message('CompactEnsemble2:aggregatePredict:UseObsForIter', N, T));
            end
            
            % Check useDfort
            if ~islogical(useDfort) || ~all([D T]==size(useDfort))
                error(message('CompactEnsemble2:aggregatePredict:UsePredForIter', D, T));
            end
        ...
        function v = aggregateLoss(T,X,Y,W,cost,funloss,combiner,fpredict,...
            ...
            elseif strncmpi(mode,'individual',length(mode))
                % Make sure you cast only the gpunnes and not also the type
                v = NaN(T,1,'like',double(X([])));
                for t=1:T
        ...
        function [combiner,score] = predictOneWithCache(combiner,X,t,...
            ...
            if any(useNfortT)
                ...
                if all(useNfortT)
                    if all(useDfortT)
                        if doclass
                            [~,score(:,pos)] = predict(weak,X);
                        else
                            score = predict(weak,X);
                        end
                    else
                        if doclass
                            [~,score(:,pos)] = predict(weak,X(:,useDfortT));
                        else
                            score = predict(weak,X(:,useDfortT));
                        end
                    end
                else
                    if doclass
        ...
        function [useNfort,useDfort,learners,mode] = checkAggregateArgs(T,N,D,varargin)
            ...
            if isempty(useNfort)
                ...
                if ~islogical(useNfort) || size(useNfort,1)~=N
                    error(message('CompactEnsemble2:checkAggregateArgs:BadUseObsForIter', N));
                end
            end
            ...
            if isempty(useDfort)
                ...
                if ~islogical(useDfort) || size(useDfort,1)~=D
                    error(message('CompactEnsemble2:checkAggregateArgs:BadUsePredForIter', D));
                end
            end            
            ...
            if islogical(learners)
                if ~isvector(learners) || length(learners)~=T
                    error(message('CompactEnsemble2:checkAggregateArgs:BadLogicalIndices', T));
                end
                learners = find(learners);
            end
            if ~isempty(learners) && ...
                    (~isnumeric(learners) || ~isvector(learners) || min(learners)<=0 || max(learners)>T)
                error(message('CompactEnsemble2:checkAggregateArgs:BadNumericIndices', T));
            end
            ...
            if ~ischar(mode) || ...
                ...
                error(message('CompactEnsemble2:checkAggregateArgs:BadMode'));
            end
        end        
    end
