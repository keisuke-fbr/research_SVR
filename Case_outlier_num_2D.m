...
for num_i = num
    ...
    for std_i = std
        ...
        % 外れ値の追加
        rng default;
        outlier = randsample(num_i, num_i*0.01); % 3%のデータのindexを選ぶ
        Y_noize(outlier) = Y_noize(outlier) + 50;

        % disp(append('(データ数=', string(num_i), ', 外れ値有りのとき)'));
        ...
        xlabel('$x$', 'FontSize',18,'Interpreter','latex'); % x軸ラベルの大きさ
        ylabel('$y$', 'FontSize',18,'Interpreter','latex'); % y軸ラベルの大きさ
        % saveas(gcf, append('人工データ(データ数=', string(num_i), ', 外れ値有りのとき).jpg')) % このコードでグラフを画像ファイルとして保存
        %}
        ...
        saveas(gcf, append('L1損失SVRの予測精度(データ数=', string(num_i), ', 外れ値有りのとき).jpg')) % このコードでグラフを画像ファイルとして保存
        %}

        disp(append('L1損失SVRの予測精度(データ数=', string(num_i), ', 外れ値有りのとき)'));
        disp("真値との平均絶対値誤差");
        ...
        ylabel('pred', 'FontSize',18); % y軸ラベルの大きさ
        % title("L2損失SVR");
        % saveas(gcf, append('L2損失SVRの予測精度(データ数=', string(num_i), ', 外れ値有りのとき).jpg')) % このコードでグラフを画像ファイルとして保存
        %}

        disp(append('L2損失SVRの予測精度(データ数=', string(num_i), ', 外れ値有りのとき)'));
        disp("真値との平均絶対値誤差");
