classdef RegressionSVM2 < ...
        FullRegressionModel2 & CompactRegressionSVM2
...
%   Copyright 2015-2020 The MathWorks, Inc.
    ...
    methods(Hidden)
        function this = RegressionSVM2(X,Y,W,modelParams,dataSummary,responseTransform)
            if nargin~=6 || ischar(W)
                 error(message('stats:RegressionSVM2:RegressionSVM2:DoNotUseConstructor'));
            end
            ...
            this = this@FullRegressionModel2(...
                X,Y,W,modelParams,dataSummary,responseTransform);
            this = this@CompactRegressionSVM2(...
            ...
            this.Impl = SVMImpl2.make(...
                ...
                this.OrdinalPredictors,...
                this.VariableRange,...
                this.ModelParams.RemoveDuplicates);
        end
        
        function newobj = update(this,varargin)
            % dispatch to compact model method
            newobj = update@CompactRegressionSVM2(this,varargin{:});
        end
    end

    methods(Static,Hidden)
        function this = fit(X,Y,varargin)
            temp = RegressionSVM2.template(varargin{:});
            this = fit(temp,X,Y);
        end
        
        function temp = template(varargin)
            FitTemplate2.catchType(varargin{:});
            temp = FitTemplate2.make('SVM','type','regression',varargin{:});
        end
    end
    ...
    methods(Access=protected)
        ...
        function s = propsForDisp(this,s)
            s = propsForDisp@CompactRegressionSVM2(this,s);
    ...
    methods
        ...
        %   See also fitrsvm, RegressionSVM, predict.
            ...
            [varargout{1:nargout}] = ...
                resubPredict@FullRegressionModel2(this,varargin{:});
       end
        ...
        %   See also RegressionSVM, loss.
            ...
            [varargout{1:nargout}] = ...
                resubLoss@FullRegressionModel2(this,varargin{:});
        end
        ...
        %   See also fitrsvm, RegressionSVM, Solver.
            ...
            this.Impl = resume(this.Impl,this.PrivX,this.PrivY,numIter,...
                doclass,verbose,nprint,...
        ...
        %   classreg.learning.partition.RegressionPartitionedSVM.
            ...
            idxBaseArg = find(ismember(varargin(1:2:end),...
                FitTemplate2.AllowedBaseFitObjectArgs));
            ...
            temp = FitTemplate2.make(this.ModelParams.Method,...
     ...
     methods(Static,Hidden)
        function [X,Y,W,dataSummary,responseTransform] = prepareData(X,Y,varargin)
            ...
            [X,Y,W,dataSummary] = ...
                FullClassificationRegressionModel2.prepareDataCR(X,Y,crArgs{:});
            ...
            [X,Y,W,dataSummary] = ...
                FullRegressionModel2.removeNaNs(X,Y,W,dataSummary);
            % Renormalize weights
            W = W/sum(W);
